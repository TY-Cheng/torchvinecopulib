...
class BB5(BiCopExtremeValue):
    ...
    def pickands_dep_func_derivative(vec: torch.Tensor, par: tuple[float]) -> torch.Tensor:
        ...
        delta_neg = -delta
        t_theta, t_1_theta = vec.pow(theta), (1 - vec).pow(theta)
        t_theta_delta_neg, t_1_theta_delta_neg = t_theta.pow(delta_neg), t_1_theta.pow(delta_neg)
    ...
    def pickands_dep_func_derivative_2(vec: torch.Tensor, par: tuple[float]) -> torch.Tensor:
        ...
        return (
            ...
            * (
                (
                    -(1 - vec).pow(theta_1) + vec.pow(theta_1) + sum_3 / sum_1.pow(1 + delta_frac)
                ).square()
                * (-theta_1)
                + sum_2
                * (
                    ...
                    + (
                        ((1 - vec).pow(-1 - theta_delta_1) + vec.pow(-1 - theta_delta_1))
                        * theta_delta_1
                    )
