...
@pytest.mark.skipif(
    not torch.cuda.is_available() or not RUN_CUDA_TESTS, reason="CUDA memory suite is opt-in"
)
    ...
    for grid_size in (65, 129):
        ...
        sizes.append(
            sum(
                ...
                for buf in [cop._pdf_grid, cop._cdf_grid, cop._hfunc_l_grid, cop._hfunc_r_grid]
            )
        )
