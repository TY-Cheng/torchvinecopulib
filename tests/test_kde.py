...
from torchvinecopulib.bicop import TorchCopulaKDE2D
from torchvinecopulib.util import TorchKDE1D
...
def test_torch_copula_kde_integral_and_shapes():
    ...
    kde = TorchCopulaKDE2D(obs=obs, num_step_grid=129)
...
def test_torch_copula_kde_boundary_mass_is_finite():
    ...
    kde = TorchCopulaKDE2D(obs=edge, num_step_grid=65)
...
def test_compile_friendly_query_paths():
    ...
    kde = TorchKDE1D(x, num_step_grid=129).to(DEVICE)
...
def test_recursive_smoother_smoke():
    ...
    kde = TorchKDE1D(x, num_step_grid=129, smoother="recursive").to(DEVICE)
    vals = kde.pdf(x[:16])
