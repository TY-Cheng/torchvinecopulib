...
setup: require-external-env
    uv sync --extra cpu --extra reference --group docs --group examples

test mode="auto": require-external-env
    @case "{{ mode }}" in \
      auto|default) \
...
examples mode="write": require-external-env
    @case "{{ mode }}" in \
      write) \
