...
def train_vine(Z_train, Y_train, seed=42, device="cpu"):
    ...
    model_vine: tvc.VineCop = tvc.VineCop(
        num_dim=ZY_train.shape[1], is_cop_scale=False
    ).to(device=device)
    model_vine.fit(
        obs=ZY_train,
...
def vine_pred_intvl(model: tvc.VineCop, Z_test, alpha=0.05, seed=42, device="cpu"):
    ...
    dct_v_s_obs[model.sample_order] = torch.full(
        (num_sample, 1), alpha / 2, device=device
    )
    lower = model.sample(
    ...
    dct_v_s_obs[model.sample_order] = torch.full(
        (num_sample, 1), 1 - alpha / 2, device=device
    )
    upper = model.sample(
        num_sample=num_sample,
        sample_order=sample_order,
